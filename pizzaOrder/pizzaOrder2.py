...
class dominosPizzaOrder():
    ...
    pizzaToppings = ["Onion", "Tomato", "Capsicum", "CottageCheeze", "Jalapeno",
        "Mushroom", "BlackOlives", "RedPaprika", "GoldenCorn", "Babycorn"]
    OrderStatus = ["recieved", "preparation",
        "baking", "outForDelivery", "Delivered"]
    ...
    def __init__(self):
        ...
        if self.pizzaCrust not in dominosPizzaOrder.pizzaCrusts:
            self._raiseException(
                self.pizzaCrust, dominosPizzaOrder.pizzaCrusts)
        ...
        for x in self.toppings:
            if x not in dominosPizzaOrder.pizzaToppings:
                self._raiseException(
                    self.toppings, dominosPizzaOrder.pizzaToppings)
    ...
    def getOrderStatus(self):
        return dominosPizzaOrder.OrderStatus[self.orderStatusIndex]

    def incrementOrderIndex(self):
        return self.orderStatusIndex += 1
